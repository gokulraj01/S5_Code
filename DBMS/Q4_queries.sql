...
SELECT employee_name
FROM employee, works
WHERE employee.employee_name = works.employee_name AND company_name != 'ABC';
...
SELECT employee.employee_name FROM employee, works
WHERE employee.employee_name = works.employee_name AND salary > ALL (
	SELECT salary
	FROM employee, works
	WHERE employee.employee_name = works.employee_name AND company_name = 'Google'
);
...
WHERE salary > (
	SELECT avgpay FROM (SELECT company_name, AVG(salary) AS avgpay FROM works GROUP BY company_name) w2
